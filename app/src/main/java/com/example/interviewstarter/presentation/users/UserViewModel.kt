    ...
    fun loadUsers() {
        viewModelScope.launch {
            ...
            _uiState.value = when (val result = getUsers()) {
                ...
                is NetworkResult.HttpError -> UserUiState(error = "HTTP ${result.code}: ${result.message}")
                is NetworkResult.NetworkError -> UserUiState(error = result.message)
                is NetworkResult.UnknownError -> UserUiState(error = result.message)
            }
        }
    }
}
